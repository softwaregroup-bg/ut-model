...
const wopts = ({WTF: opts.WTF, bookSST: program.sst}/*: any */);
if (program.compress) wopts.compression = true;

wopts.dateNF = 'yyyy-mm-dd HH:MM:ss';
opts.cellDates = true;
opts.cellText = false;
...
switch (true) {
    ...
    default:
        if (!program.book) {
            const stream = X.stream.to_csv(ws, {FS: program.fieldSep, RS: program.rowSep});
